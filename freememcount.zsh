...
#echo ${freeoutcontent}
# 取mem字符串并括起来再次形成数组
memcontent=(${freeoutcontent[7,13]})
#echo ${(t)memcontent} $memcontent
# 取swap字符串必能括起来再次形成数组
swapcontent=(${freeoutcontent[14,17]})
#echo ${(t)swapcontent} $swapcontent

# 指定数值类型为浮点数float，否则默认是整数类型
float totalmemo
totalmemo=(${memcontent[2]})
float freememo=$((${memcontent[-1]}))
#echo $freememo $totalmemo
...
print $(( int($percent) ))
# 装配待输出的成型字符串，用\t分隔
itemfree="$(date "+%s")\t$(( int($percent) ))\t${swapcontent[2]}\t${swapcontent[-1]}"
#print $itemfree
zmodload -i zsh/mapfile
# 判断文件是否存在的语句：${+mapfile[$datafile]}
dfexistsed=${+mapfile[$datafile]}
#echo $dfexistsed ${(t)dfexistsed}
#echo $(($dfexistsed == 0))
if ((dfexistsed == 0)); then
	print "$datafile 不存在，需要构建新的相关目录和文件"
	# 取得文件名字符串中目录部分的语法
	dirstr=${datafile:h}
	if [[ ! -d $dirstr ]]; then
		mkdir $dirstr -p
...
#echo $specline ${#specline} ${specline[(i)\=]}
...
if [[ $1 == 'convert' ]]; then
	...
	if [ ${#fileconarray[*]} != 0 ]; then
		...
		print "首行（其实是数组第一个item）内容为：\t${fileconarray[1]}"
	...
	while (( $ii <= $linescount )){
		...
		tdsa=(${linecontent[1,6]})
		timestr="${tdsa[-1]}-${monthdict[${tdsa[2]}]}-${tdsa[3]} ${tdsa[4]}"
		# 指定数值类型为浮点数float，否则默认是整数类型
		float totalmemo=$((${linecontent[7]}))
		float freememo=$((${linecontent[8]}))
		# 数值计算方式$(( $var1 * $var2 ))
		percent=$(($freememo * 100 / $totalmemo ))
		print $(date --date="$timestr" "+%s")"\t$(( int($percent) ))\t${linecontent[9]}\t${linecontent[10]}" >> $datafilebak
		ii+=1
	}
fi
